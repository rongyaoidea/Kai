...
OpenCode's gateway expects the full official client header set and rejects free-model requests that arrive without it ("only usable in OpenCode"): `x-opencode-client: cli`, `x-opencode-session`, `x-opencode-project`, and a fresh `x-opencode-request` per HTTP request. Kai sends the current conversation id as the session id, so one chat -- however many turns, tool round-trips or bailout retries it takes -- reads as a single session upstream, and separate chats read as separate sessions. Requests that belong to no conversation (fetching the model list, connection validation) carry a session id generated once per app process instead. `x-opencode-project` is likewise a stable per-process id (Kai has no project concept); `x-opencode-request` is a fresh random id on every HTTP call, mirroring the official client's per-message id. The headers are sent on every OpenCode request shape: chat completions, the Responses API, Messages, and the model list.
...
Zen's free-model pool gates on both the header set and the User-Agent rather than key or IP: only `opencode/<version>` (currently `opencode/1.18.31`, tracking the latest official release) gets 200s — anything else, or the headers alone without the official User-Agent, is rejected. Kai therefore overrides its default User-Agent with the official one on Zen requests only; every other provider keeps reporting Kai honestly. Go endpoints are the deliberate exception: Go asks clients to identify with their own user agent rather than a generic name and does not gate on the official one, so Go requests keep Kai's own User-Agent and gain only the client headers. Separately, keyed Zen enforces a silent ~15-20 RPM burst limit with no Retry-After headers — the per-minute request cap in Services settings exists exactly for pacing past that wall.
