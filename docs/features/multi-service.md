...
- **Messages** (`/messages`, Anthropic shape): Zen's Claude and Qwen rows; Go additionally serves its MiniMax rows there (Zen serves the same MiniMax ids on chat completions, so the Zen and Go base URLs select different lists). Auth is the gateway bearer key with the usual session header, plus Anthropic-native `x-api-key` and `anthropic-version` headers (the official client speaks raw Anthropic here; bearer alone answers 401); the request/response handling is the same Anthropic loop Kai uses for Anthropic itself, minus prompt-caching breakpoints. Selected by `requiresMessagesApi`.
...
OpenCode's gateway expects the full official client header set and rejects free-model requests that arrive without it ("only usable in OpenCode"): `x-opencode-client: cli`, `x-opencode-session`, `x-opencode-project`, and a fresh `x-opencode-request` per HTTP request. Since 2026-09-16 the session id must additionally have the official shape (`ses_` + 12 lowercase hex + 14 Base62 characters); any other shape is rejected with the same 403 regardless of the other headers. Kai derives that shape deterministically from the conversation id — same chat, same session, across restarts — so one chat, however many turns, tool round-trips or bailout retries it takes, reads as a single upstream session and separate chats stay separate. Requests that belong to no conversation (fetching the model list, connection validation) carry a session id minted once per app process instead. `x-opencode-project` is likewise a stable per-process id (Kai has no project concept); `x-opencode-request` is a fresh random id on every HTTP call, mirroring the official client's per-message id. The headers are sent on every OpenCode request shape: chat completions, the Responses API, Messages, and the model list.

The id values are Kai's own random identifiers; nothing about the user or the machine is derived from them. No other provider is sent these headers -- except an OpenAI-Compatible API instance pointed at an opencode.ai base URL (the pre-preset way to reach Go): Go requires the same headers for routing and prompt caching, so those requests carry them too.

Zen's free-model pool gates on the header set, the User-Agent, and the request body rather than key or IP. The User-Agent must be `opencode/<version>` (currently `opencode/1.18.31`, tracking the latest official release); anything else, or the headers alone without it, is rejected. The body must look like an agent session too: `stream: true`, plus the five core tool names (`bash`, `edit`, `glob`, `grep`, `read`) declared in `tools`. Kai satisfies this for free models only, by forcing streaming and appending any missing core tool declaration — cloning the schema of the matching Kai tool where one exists (`bash`/`read`/`edit` map to the shell and file tools), otherwise a parameter-less placeholder whose stray call answers with guidance. Free models that lack a matching Kai tool simply get the placeholder. Streaming is consumed internally and assembled into the same response shape the non-streaming path returns, so the tool loop, history, and UI are unaware of it; a long generation is allowed to outlive the usual 180s request budget by relying on the socket timeout, which every SSE frame resets. A 403 that carries the gateway's `FreeTierError` is surfaced as its own user-facing message instead of being misreported as content moderation. Every other provider keeps its non-streaming bodies and Kai's honest User-Agent — Go included: Go asks clients to identify with their own user agent rather than a generic name and does not gate on the official one, so Go requests keep Kai's own User-Agent and gain only the client headers. Separately, keyed Zen enforces a silent ~15-20 RPM burst limit with no Retry-After headers — the per-minute request cap in Services settings exists exactly for pacing past that wall.

To use Go models, add the built-in **OpenCode Go** service and paste the API key from the Zen console; headers and per-family endpoint routing are applied automatically. (Before the preset existed, the path was an OpenAI-Compatible API service with base URL `https://opencode.ai/zen/go/v1` — that still works.) Zen's free models (the `-free` suffixed ids in the Zen model list, plus stealth drops like `big-pickle`) work through the built-in OpenCode service in the shaped form above; they are marked with the Free badge in the model picker.
...
| OpenCode | `opencode` | Yes | OpenAI-compatible (every request carries the `x-opencode-*` client headers — see [Session Header](#session-header-opencode); models route per family across chat completions, Responses and Messages — see [Gateway endpoint routing](#gateway-endpoint-routing-opencode)) |
...
- **Free badge** — a green "Free" label next to the title when the model is on that provider's free tier according to the curated free-tier catalog (currently **Ollama Cloud**, **OpenRouter**, and **OpenCode Zen**). Free-ness is per service, not global; Zen's list is suffix-driven (`-free`) plus the known stealth ids.
...
| `composeApp/src/commonMain/.../data/FreeTierModels.kt` | Runtime free-tier model ids per service (Ollama Cloud, OpenRouter, OpenCode Zen) |
| `docs/knowledge/free-tier/` | OKF bundle: free-tier policy, snapshot, sources, refresh playbook |
| `composeApp/src/commonMain/.../network/OpenCodeIds.kt` | Canonical `ses_`/`msg_`/`prj_` ids for the OpenCode gateway |
| `composeApp/src/commonMain/.../network/ZenAgentShape.kt` | Free-model detection plus the streaming/core-tool body shape Zen's free pool requires |
| `composeApp/src/commonMain/.../network/OpenAISse.kt` | SSE reader and stream-to-DTO accumulators for the free-model streaming path |
