...
| `read_file` / `write_file` (Android) | Paginated text reads and exact-mode writes (overwrite/append/create) in the workspace — sandbox `/root` when installed, the native workspace otherwise | Follows the sandbox switch, no switch of their own |
...
#### No-key search chain (evaluated 2026-09-11, CN sources added 2026-09-16)

`web_search` is keyless and scrapes public result pages. Sources race in parallel with a 10s budget each; the chain's budget is 12s. The reader returns the best result it can no longer improve on instead of waiting for everyone: the **primary group** (the first three sources in priority order) gets 6s, higher-priority group members get a 1.5s grace window to beat a lower-priority result that already arrived, and the tail sources only matter when the group yields nothing. Non-2xx responses count as failures (with the status code), verification pages are classified separately, and results never parse to silent empties.

Priority is query-aware. Queries containing Han characters search **Baidu → Bing → Sogou → 360** first, because that is what works in mainland China without a VPN; DuckDuckGo (unreachable there) drops behind them. Everything else keeps the long-standing **DuckDuckGo → Bing → Marginalia** order. When a `time` filter is set, the Baidu News RSS feed — inherently recency-sorted — slots in right after the leading source.

1. **DuckDuckGo** — instant-answer API plus the html and lite endpoints. Long-standing default for non-Chinese queries; honors `time` (`day`/`week`/`month` → DDG's `df`).
2. **Bing** — `b_algo` result blocks; `/ck/a` redirects decoded from the base64url `u=a1…` payload. Reachable in China (the CN IP is served the China edition) and the best English index available there. Honors `time` via `filters=ex1:"ez1|ez2|ez3"`.
3. **Marginalia** — independent index, strong on long-tail and non-commercial content.
4. **Baidu** — organic `result c-container` blocks (ads are `result-op` and skipped); `/link?url=` redirects are returned as-is and followed by `fetch_url`. Chinese queries' best source; verification pages (`wappass`) are classified as captchas so telemetry doesn't lie.
5. **Sogou** — `vrwrap` result blocks; a session cookie is fetched from its homepage once per process because Sogou intermittently requires one. The only entry to WeChat-article search.
6. **360** — `res-list` blocks; historically the most scraper-tolerant of the Chinese engines.
7. **Yandex** — reachable in China and the only direct-connect engine that does not apply Chinese-style content filtering; slow and captcha-prone, so it sits last.
8. **Baidu News RSS** — `news.baidu.com/ns?tn=newsrss`, plain XML items; only raced when the caller passed a `time` filter.

**Fail-fast bench:** a source that proves unreachable is skipped for a cooldown instead of costing every later search its budget — two consecutive network failures bench it for 10 minutes, two HTTP failures for 30, and one captcha/verification page for 60. A success clears the bench. The bench is per-process (an app restart retries everything, so a VPN or network change recovers without user action), and `source_status` reports benched sources as `skipped: benched (~Ns)`.

Evaluated and rejected: Mojeek (captcha wall), Startpage (Anubis proof-of-work challenge), Ecosia (403 to scrapers), SearXNG public instances (instance lottery), Toutiao search (requests require `a_bogus`/`msToken` signing plus a logged-in cookie — reverse-engineering cost is not justified), Sogou WeChat search endpoint (cookie + frequent captchas), Zhihu/Weibo/Xiaohongshu (login walls).
...
Where `browse_page` reads, `web_act` operates: each conversation holds a persistent offscreen browser session (page, history, cookies) for multi-step flows like search → open → fill → submit. Element ids work like the app-automation tree — `snapshot` first, act on fresh ids, re-snapshot when the page reports a change. Ids are the snapshot's own element indices (hidden/tiny interactive elements are skipped, so ids can have gaps but always address the element the snapshot showed), and both tap and input accept an optional `expect` text from the snapshot: the action is refused with the element's current text when the page changed underneath, instead of acting on the wrong element. Scrolling and snapshots target the page's real scroll container (inner `overflow` containers, not just the window) and report `scroll_y` / `max_scroll_y` / `at_bottom`, so the agent knows whether more content exists below and whether a scroll actually moved the page. Irreversible actions and credential entry follow the same confirm-first policy as the app-automation tools. Deliberately Kai-native (no browser-use framework, no extra model, no keys): the main agent tool loop drives, so approvals and timeouts behave like every other tool.
...
The `open_file` tool fires an `ACTION_VIEW` Intent for a single file at `path` (relative to the workspace home in both tiers; the `/root/` prefix is also accepted since the shell reports paths that way). Browser opens HTML, image viewer opens PNG/JPG, PDF viewer opens PDF, etc. Uses the existing FileProvider declared in the main manifest. Path is rejected if it contains a leading slash or `..` segments; the resolved path is canonicalized and verified to stay under the workspace home (or browser-screenshot cache) before the Intent is fired. `preview=true` on an HTML file instead publishes it to the chat as an inline report card — tapping the card opens a locked-down full-screen WebView (JS on, file/content access off, navigation cancelled), so reports stay inside Kai; unsupported platforms, non-HTML files, and documents over 512 KB silently fall back to the external-open path.
...
`open_file` operates on a single file. FileProvider grants access only to the specific URI in the Intent, so it does not work for multi-file content (e.g. an HTML page that loads sibling `styles.css` / `script.js`). For HTML, write self-contained files with inline CSS and JavaScript; the shell and `open_file` tool descriptions tell the agent to do this. The in-app preview injects the document as a string rather than loading `file://`, so the same self-contained rule applies to it.
...
- **Sandbox install gate (Android)** — `ssh_configure_host` is surfaced only when the Linux sandbox is actually installed (Ready) *and* the sandbox toggle is on. `execute_shell_command`, `manage_process`, `read_file`, `write_file` and `open_file` are tiered instead of gated: full proot Linux when Ready, host mksh/toybox plus the app-private native workspace (no rootfs, no download) otherwise — same tool surface and session model, narrower capabilities. Skills work in both tiers too: sandbox folders when Ready, the native workspace otherwise, degrading to prompt-only instructions.
