...
- **Tool availability follows install state — except the shell itself**: the file tools (`read_file`, `write_file`), `ssh_configure_host`, and skills are advertised to the model only when the sandbox is actually installed (Ready) *and* the sandbox toggle is on. `execute_shell_command` and `manage_process` instead fall back to the host mksh/toybox tier when Ready is absent (same surface and session table, narrower capabilities — no packages, no ssh, no scripting runtimes), so there is no state in which the agent has zero shell. The enable/disable toggle is itself hidden until install completes.
